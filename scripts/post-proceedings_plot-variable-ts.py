...
def get_curve_labelOLD(filename):
    # mapping = {
...
def plot_cdfs_from_folder(folder, parallelValue, seqValue, alternativeValue):
    ...
    filenames = [ 'GT_'+ expID + '.txt' , 'pyramisCDF_' + expID + '_ts0.4.txt', 'pyramisCDF_' + expID +'_ts0.2.txt', 'pyramisCDF_' + expID +'_ts0.1.txt' ]
    curves = []
...
if __name__ == "__main__":
    ...
    plot_cdfs_from_folder(args.folder, 2, 2, 2) 
    ...
    for i, (param_name, param_value) in enumerate(combinations, 1):
        ...
        plot_cdfs_from_folder(args.folder, kwargs['PAR'], kwargs['SEQ'], kwargs['ALT'])
